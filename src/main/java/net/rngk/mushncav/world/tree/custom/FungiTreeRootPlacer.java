...
import net.rngk.mushncav.world.tree.ModRootPlacerTypes;

public class FungiTreeRootPlacer extends RootPlacer {
    public static final int field_38769 = 8;
    public static final int field_38770 = 15;
    public static final Codec<FungiTreeRootPlacer> CODEC = RecordCodecBuilder.create((instance) -> {
        return method_43182(instance).and(FungiTreeRootPlacement.CODEC.fieldOf("fungi_tree_root_placement").forGetter((rootPlacer) -> {
            return rootPlacer.fungiTreeRootPlacement;
        })).apply(instance, FungiTreeRootPlacer::new);
    });
    private final FungiTreeRootPlacement fungiTreeRootPlacement;

    public FungiTreeRootPlacer(IntProvider trunkOffsetY, BlockStateProvider rootProvider, Optional<AboveRootPlacement> aboveRootPlacement, FungiTreeRootPlacement fungiTreeRootPlacement) {
        super(trunkOffsetY, rootProvider, aboveRootPlacement);
        this.fungiTreeRootPlacement = fungiTreeRootPlacement;
    }
    ...
    public boolean canGrow(TestableWorld world, Random random, BlockPos pos, Direction direction, BlockPos origin, List<BlockPos> offshootPositions, int rootLength) {
        int i = this.fungiTreeRootPlacement.maxRootLength();
    ...
    public List<BlockPos> getOffshootPositions(BlockPos pos, Direction direction, Random random, BlockPos origin) {
        BlockPos blockPos = pos.down();
        BlockPos blockPos2 = pos.offset(direction);
        int i = pos.getManhattanDistance(origin);
        int j = this.fungiTreeRootPlacement.maxRootWidth();
        float f = this.fungiTreeRootPlacement.randomSkewChance();
        if (i > j - 3 && i <= j) {
    ...
    public boolean canGrowThrough(TestableWorld world, BlockPos pos) {
        return super.canGrowThrough(world, pos) || world.testBlockState(pos, (state) -> {
            return state.isIn(this.fungiTreeRootPlacement.canGrowThrough());
        });
    }

    public void placeRoots(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, BlockPos pos, TreeFeatureConfig config) {
        if (world.testBlockState(pos, (state) -> {
            return state.isIn(this.fungiTreeRootPlacement.muddyRootsIn());
        })) {
            BlockState blockState = this.fungiTreeRootPlacement.muddyRootsProvider().get(random, pos);
    ...
    public RootPlacerType<?> getType() {
        return ModRootPlacerTypes.FUNGI_TREE_ROOT_PLACER;
    }
}